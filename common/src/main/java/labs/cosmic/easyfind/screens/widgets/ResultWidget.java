...
public class ResultWidget extends AbstractSelectionList<ResultWidget> {
    ...
    public void render(GuiGraphics context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
        ...
        int color = ConfigAgent.coloredRarity ? rarity.getColor() : 0xFFFFFF;
        if (isEnabled) {
            text = Component.translatable(item.getDescriptionId()).withStyle(ChatFormatting.byColor(color));
        } else {
        ...
        itemRenderer.renderAndDecorateItem(itemStack, x + 2, y + 2);
